...
module rv32i_mem_bus_ahb_tb;
  ...
  initial begin
    for (idx = 0; idx < 256; idx = idx + 1) begin
      rom[idx]  = 32'h1000_0000 + idx;
      sram[idx] = 32'h2000_0000 + idx;
    end
  ...
  always @(posedge clk) begin
    if (rom_valid && rom_ready && rom_write) begin
      $fatal(1, "unexpected ROM write through AHB bus");
    end
  ...
  task automatic wait_i_ready;
    begin
      timeout = 0;
      while (!i_ready && (timeout < 40)) begin
        @(posedge clk);
        #1ps;
        timeout = timeout + 1;
      end
      if (!i_ready) begin
        $fatal(1, "timeout waiting for AHB I ready");
      end
    end
  endtask
  ...
  task automatic wait_d_ready;
    begin
      timeout = 0;
      while (!d_ready && (timeout < 80)) begin
        @(posedge clk);
        #1ps;
        timeout = timeout + 1;
      end
      if (!d_ready) begin
        $fatal(1, "timeout waiting for AHB D ready");
      end
    end
  endtask
  ...
  task automatic i_read_check(
    ...
    begin
      ...
      i_valid = 1'b1;
      wait_i_ready();
      if (i_error || (i_rdata !== expected)) begin
        $fatal(1, "AHB I read mismatch at 0x%08x: error=%0d got=0x%08x expected=0x%08x",
               addr, i_error, i_rdata, expected);
      end
      i_valid = 1'b0;
      @(posedge clk);
      #1ps;
  ...
  task automatic d_read_check(
    ...
    begin
      ...
      d_valid = 1'b1;
      wait_d_ready();
      if (d_error || (d_rdata !== expected)) begin
        $fatal(1, "AHB D read mismatch at 0x%08x: error=%0d got=0x%08x expected=0x%08x",
               addr, d_error, d_rdata, expected);
      end
      d_valid = 1'b0;
      @(posedge clk);
      #1ps;
  ...
  task automatic d_write_check(
    ...
    begin
      ...
      wait_d_ready();
      if (d_error) begin
        $fatal(1, "AHB D write returned error at 0x%08x", addr);
      end
      d_valid = 1'b0;
      d_write = 1'b0;
      @(posedge clk);
  ...
  initial begin
    ...
    d_write_check(32'h2000_0008, 32'hAA_BB_CC_DD, 4'b0101);
    d_read_check(32'h2000_0008, 32'h20BB_00DD);

    d_write_check(32'h4000_0000, 32'hCAFE_BABE, 4'b1111);
    ...
    wait_d_ready();
    if (i_ready) begin
      $fatal(1, "AHB I bus should not complete in the same grant as D bus");
    end
    if (d_rdata !== 32'h2000_0000) begin
      $fatal(1, "AHB D priority read mismatch: got 0x%08x", d_rdata);
    end
    d_valid = 1'b0;
    wait_i_ready();
    if (i_rdata !== 32'h1000_0000) begin
      $fatal(1, "AHB I read after D priority mismatch: got 0x%08x", i_rdata);
    end
    i_valid = 1'b0;
    @(posedge clk);
    #1ps;
    ...
    d_valid = 1'b1;
    wait_d_ready();
    if (!dbg_decode_error || !d_error || (d_rdata !== 32'd0)) begin
      $fatal(1, "expected AHB D decode error: dbg=%0d err=%0d rdata=0x%08x",
             dbg_decode_error, d_error, d_rdata);
    end
    d_valid = 1'b0;
    @(posedge clk);
    #1ps;
    ...
    i_valid = 1'b1;
    wait_i_ready();
    if (!dbg_decode_error || !i_error || (i_rdata !== 32'd0)) begin
      $fatal(1, "expected AHB I decode error: dbg=%0d err=%0d rdata=0x%08x",
             dbg_decode_error, i_error, i_rdata);
    end
    i_valid = 1'b0;
    @(posedge clk);
    #1ps;
    ...
    if (dbg_i_grant_count < 32'd2) begin
      $fatal(1, "expected at least two AHB I grants, got %0d", dbg_i_grant_count);
    end
    if (dbg_d_grant_count < 32'd6) begin
      $fatal(1, "expected at least six AHB D grants, got %0d", dbg_d_grant_count);
    end

    $display("[PASS] rv32i_mem_bus_ahb_tb");
    $display("  i_grants=%0d d_grants=%0d", dbg_i_grant_count, dbg_d_grant_count);
    $display("  AHB-Lite bridge/decode, D-priority and byte-split writes passed");
    $finish;
  end
