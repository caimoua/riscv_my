...
module rv32i_cached_access_fault_tb;
  ...
  initial begin
    ...
    rom[2] = 32'h8000_00b7; // lui   x1, 0x80000      (unmapped base)
    rom[3] = 32'h0000_a103; // lw    x2, 0(x1)        (load access fault)
    ...
    rom[7] = 32'h0150_a223; // sw    x21, 4(x1)       (store access fault)
    rom[8] = 32'h0660_0b13; // addi  x22, x0, 0x66    (after store handler)

    // Load access fault handler at 0x140.
    ...
    // Store access fault handler at 0x180.
  ...
  always @(posedge clk) begin
    if (rom_valid && rom_ready && rom_write) begin
      $fatal(1, "unexpected ROM write in cached access fault test");
    end

    if (mmio_valid) begin
      $fatal(1, "unexpected MMIO access in cached access fault test");
    end
  ...
  initial begin
    ...
    while ((!load_handler_done || !store_handler_done ||
            !load_resume_done || !store_resume_done) &&
           (timeout < 1000)) begin
      @(posedge clk);
      #1ps;
      dbg_reg_addr = 5'd31;
    ...
    if (!load_handler_done || !store_handler_done ||
        !load_resume_done || !store_resume_done) begin
      $fatal(1, "timeout waiting for load/store access fault handlers and resumes");
    end
    if (dbg_illegal_instr) begin
      $fatal(1, "unexpected illegal instruction in cached access fault test");
    end
    if (dbg_ecall) begin
      $fatal(1, "unexpected ECALL event in cached access fault test");
    end
    if (dbg_ebreak) begin
      $fatal(1, "unexpected EBREAK event in cached access fault test");
    end
    if (!bus_decode_error_seen) begin
      $fatal(1, "expected bus decode error while accessing unmapped address");
    end

    check_reg(5'd1,  32'h8000_0000, "x1");
    check_reg(5'd2,  32'd0,         "x2 (faulting load must not write back)");
    check_reg(5'd21, 32'h55,        "x21 (load fault resume)");
    check_reg(5'd22, 32'h66,        "x22 (store fault resume)");
    check_reg(5'd30, 32'd1,         "x30 (store fault handler marker)");
    check_reg(5'd31, 32'd1,         "x31 (load fault handler marker)");

    if (sram[0] !== `RV32I_TRAP_CAUSE_LOAD_ACCESS_FAULT) begin
      $fatal(1, "load fault mcause mismatch: expected 5, got 0x%08x", sram[0]);
    end
    if (sram[1] !== 32'h0000_000c) begin
      $fatal(1, "load fault mepc mismatch: expected 0x0000000c, got 0x%08x", sram[1]);
    end
    if (sram[2] !== `RV32I_TRAP_CAUSE_STORE_ACCESS_FAULT) begin
      $fatal(1, "store fault mcause mismatch: expected 7, got 0x%08x", sram[2]);
    end
    if (sram[3] !== 32'h0000_001c) begin
      $fatal(1, "store fault mepc mismatch: expected 0x0000001c, got 0x%08x", sram[3]);
    end
    ...
    $display("[PASS] rv32i_cached_access_fault_tb");
    $display("  pc=0x%08x cycle=%0d instret=%0d stall_cycle=%0d",
             dbg_pc, dbg_cycle, dbg_instret, dbg_stall_cycle);
    $display("  load_fault:  mcause=0x%08x mepc=0x%08x",
             sram[0], sram[1]);
    $display("  store_fault: mcause=0x%08x mepc=0x%08x",
             sram[2], sram[3]);
    ...
    $display("  unmapped load/store produced precise access fault traps and resumed with MRET");
    $finish;
  end
