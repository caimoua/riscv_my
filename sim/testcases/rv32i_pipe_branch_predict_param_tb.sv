...
module rv32i_pipe_branch_predict_param_tb;
  ...
  always @(posedge clk) begin
    if (dmem_valid) begin
      $fatal(1, "unexpected data-memory access in parameterized branch predictor test");
    end
  end

  rv32i_pipe_core #(
    .BRANCH_PRED_INDEX_BITS(2)
  ) u_core (
    .clk                         (clk),
    .rst_n                       (rst_n),
    .timer_irq                   (1'b0),
  ...
  initial begin
    ...
    timeout = 0;
    while (!dbg_ebreak && (timeout < 180)) begin
      @(posedge clk);
      #1ps;
      timeout = timeout + 1;
    end
    ...
    if (!dbg_ebreak) begin
      $fatal(1, "timeout waiting for parameterized branch predictor ebreak");
    end
    if (dbg_illegal_instr) begin
      $fatal(1, "unexpected illegal instruction in parameterized branch predictor test");
    end
    if (dbg_ecall) begin
      $fatal(1, "unexpected ECALL event in parameterized branch predictor test");
    end
    ...
    if (dbg_instret !== 32'd26) begin
      $fatal(1, "instret mismatch: expected 26, got %0d", dbg_instret);
    end
    ...
    $display("[PASS] rv32i_pipe_branch_predict_param_tb");
    ...
    $display("  btb_hit=%0d btb_miss=%0d bht_update=%0d branch_pred_index_bits=2",
             dbg_btb_hit_count, dbg_btb_miss_count, dbg_bht_update_count);
    $display("  parameterized 4-entry BHT+BTB learned repeated taken branches");
    $finish;
  end
