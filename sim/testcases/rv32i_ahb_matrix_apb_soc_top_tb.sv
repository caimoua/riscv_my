...
module rv32i_ahb_matrix_apb_soc_top_tb;
  ...
  initial begin
    ...
    if (!$value$plusargs("FLASH_MEMH=%s", flash_memh)) begin
      flash_memh = "../software/bin/ahb_matrix_apb_soc.memh";
    end
  ...
  always @(posedge clk) begin
    if (flash_valid && flash_ready && flash_write) begin
      $fatal(1, "unexpected flash write in AHB matrix APB SoC test");
    end
  ...
  initial begin
    ...
    if (!dbg_ebreak) begin
      $fatal(1, "timeout waiting for AHB matrix APB SoC top ebreak");
    end
    if (dbg_illegal_instr || dbg_ecall ||
        dbg_cpu_bus_error || dbg_matrix_decode_error || dbg_apb_decode_error) begin
      $fatal(1, "unexpected trap or bus event in AHB matrix APB SoC top test");
    end
    ...
    check_reg(5'd15, 32'h22,        "x15");
    check_reg(5'd16, 32'h22,        "x16");
    check_reg(5'd17, 32'h41,        "x17");
    check_reg(5'd18, 32'h4200_1000, "x18");
    check_reg(5'd19, 32'h41,        "x19");
    check_reg(5'd20, 32'd1,         "x20");
    check_reg(5'd21, 32'h66,        "x21");
    check_reg(5'd22, 32'h63,        "x22");
    check_reg(5'd23, 32'd1,         "x23");
    ...
    if (dbg_timer_mtimecmp_lo !== 32'h22) begin
      $fatal(1, "APB timer mtimecmp_lo mismatch: expected 0x22, got 0x%08x",
             dbg_timer_mtimecmp_lo);
    end
    if (dbg_uart_tx_count !== 32'd1) begin
      $fatal(1, "APB UART tx_count mismatch: expected 1, got %0d", dbg_uart_tx_count);
    end
    if (dbg_uart_last_tx !== 8'h41) begin
      $fatal(1, "APB UART last_tx mismatch: expected 0x41, got 0x%02x",
             dbg_uart_last_tx);
    end
    if (timer_irq) begin
      $fatal(1, "timer_irq should remain low in APB SoC smoke test");
    end
    ...
    $display("[PASS] rv32i_ahb_matrix_apb_soc_top_tb");
    ...
    $display("  timer_mtimecmp_lo=0x%08x uart_tx_count=%0d uart_last_tx=0x%02x",
             dbg_timer_mtimecmp_lo, dbg_uart_tx_count, dbg_uart_last_tx);
    $display("  AHB matrix SoC reached APB timer/UART through AHB-to-APB bridge");
    $finish;
  end
