...
module rv32i_cached_system_top_tb;
  ...
  always @(posedge clk) begin
    if (rom_valid && rom_ready && rom_write) begin
      $fatal(1, "unexpected ROM write: addr=0x%08x wdata=0x%08x wstrb=0x%x",
             rom_addr, rom_wdata, rom_wstrb);
    end
  ...
  initial begin
    ...
    if (!dbg_ebreak) begin
      $fatal(1, "timeout waiting for cached system top ebreak");
    end
    if (dbg_illegal_instr) begin
      $fatal(1, "unexpected illegal instruction in cached system top test");
    end
    if (dbg_ecall) begin
      $fatal(1, "unexpected ECALL event in cached system top test");
    end
    if (dbg_bus_decode_error) begin
      $fatal(1, "unexpected bus decode error");
    end
    if (mmio_valid) begin
      $fatal(1, "unexpected MMIO access: addr=0x%08x write=%0d wdata=0x%08x wstrb=0x%x",
             mmio_addr, mmio_write, mmio_wdata, mmio_wstrb);
    end
    ...
    if ((dbg_icache_miss_count == 32'd0) || (dbg_dcache_miss_count == 32'd0)) begin
      $fatal(1, "expected both I-cache and D-cache misses");
    end

    $display("[PASS] rv32i_cached_system_top_tb");
    ...
    $display("  rv32i_cached_system_top wrapper passed");
    $finish;
  end
