module rv32i_ahb_lite_matrix_1m4s #(
  ...
  localparam [2:0] TARGET_S0    = 3'd0;
  localparam [2:0] TARGET_S1    = 3'd1;
  localparam [2:0] TARGET_S2    = 3'd2;
  localparam [2:0] TARGET_S3    = 3'd3;
  localparam [2:0] TARGET_ERROR = 3'd4;

  reg        data_valid_q;
  reg [2:0]  data_target_q;

  wire        address_valid;
  wire [2:0]  address_target;
  ...
  function [2:0] decode_target;
    ...
    begin
      ...
      end else if ((addr & S3_MASK) == S3_BASE) begin
        decode_target = TARGET_S3;
      end else begin
  ...
  assign selected_hrdata = (data_target_q == TARGET_S0) ? s0_hrdata :
                           ...
                           (data_target_q == TARGET_S3) ? s3_hrdata :
                                                          32'd0;
  assign selected_hreadyout = !data_valid_q                    ? 1'b1 :
                              ...
                              (data_target_q == TARGET_S3)    ? s3_hreadyout :
                                                                 1'b1;
  assign selected_hresp = !data_valid_q                    ? HRESP_OKAY :
                          ...
                          (data_target_q == TARGET_S3)    ? s3_hresp :
                                                             HRESP_ERROR;
  ...
  assign s3_hsel = address_valid && m_hready && (address_target == TARGET_S3);
  ...
  assign s3_haddr   = m_haddr;
  assign s3_hburst  = m_hburst;
  assign s3_hprot   = m_hprot;
  assign s3_hsize   = m_hsize;
  assign s3_htrans  = m_htrans;
  assign s3_hwdata  = m_hwdata;
  assign s3_hwrite  = m_hwrite;
  assign s3_hready  = m_hready;
